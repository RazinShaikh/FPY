...
#NOTE: the activation function used here is the softplus function, not sigmoid
@njit
def sig(x):
...
@njit
def sig2(x):
    return sig1(x) * (1 - sig1(x))

@njit
def sig3(x):
    return (sig2(x) - 2*sig1(x)*sig2(x))
...
#     dOmega = 3 * np.pi**3 / len(all_points)
    dOmega = np.pi * RADIUS**2 / len(all_points)
...
def relative_err_without_points(p00, p01, p1, p2, dx_r, dx_th):
