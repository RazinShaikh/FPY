...
@njit
def psi_a(x, y, zz):
    return (RADIUS**2 - (x**2 + y**2 + zz**2)) / 6
...
@njit
def N(x, y, zz, p00, p01, p02, p1, p2):
    return np.sum(sig(z(x, y, zz, p00, p01, p02, p1)) * p2)


# $$\frac{\partial^k N}{\partial x_j^k} = \sum_{i=1}^H v_i w_{ij}^k \sigma^{(k)}$$
@njit
def dN_dxj_k(x, y, zz, p00, p01, p02, p1, p2, j, k):
    if j==0:
        wj = p00
    elif j==1:
        wj = p01
...
@njit
def d_dwj_dN_dxk(x, y, zz, p00, p01, p02, p1, p2, j, k):
    if j==0:
        xj = x
    elif j==1:
        xj = y
...
@njit
def d_dwj_dN2_dxk2(x, y, zz, p00, p01, p02, p1, p2, j, k):
    if j==0:
        xj = x
    elif j==1:
        xj = y
...
@njit
def d_du_dkN(x, y, zz, p00, p01, p02, p1, p2, j, k):
    if j==0:
        wj = p00
    elif j==1:
        wj = p01
...
@njit
def d_dv_dkN(x, y, zz, p00, p01, p02, p1, p2, j, k):
    if j==0:
        wj = p00
    elif j==1:
        wj = p01
...
# %%
@njit
def error_term1(x, y, zz, p00, p01, p02, p1, p2):
    ans = dN_dxj_k(x, y, zz, p00, p01, p02, p1, p2, 0, 2) + \
          dN_dxj_k(x, y, zz, p00, p01, p02, p1, p2, 1, 2) + \
          dN_dxj_k(x, y, zz, p00, p01, p02, p1, p2, 2, 2) -  f(x, y, zz)
    return ans


@njit
def cost(points, boundary_points, p00, p01, p02, p1, p2, BC):
    et1 = np.zeros(points.shape[0])
    et2 = np.zeros(boundary_points.shape[0])
    
    for i, pnt in enumerate(points):
        et1[i] = error_term1(pnt[0], pnt[1], pnt[2], p00, p01, p02, p1, p2)**2
    
    for i, pnt in enumerate(boundary_points):
        et2[i] = N(pnt[0], pnt[1], pnt[2], p00, p01, p02, p1, p2)**2
...
@njit
def relative_err(p00, p01, p02, p1, p2, all_points):
    #volume/num_points
    dOmega = (4 * np.pi * RADIUS**3) / (3 * len(all_points))
    ...
    for i, pnt in enumerate(all_points):
        tr1[i] = dOmega * (np.abs(N(pnt[0], pnt[1], pnt[2], p00, p01, p02, p1, p2) - psi_a(pnt[0], pnt[1], pnt[2]))**2)

    for i, pnt in enumerate(all_points):
        tr2[i] = dOmega * ((dN_dxj_k(pnt[0],pnt[1],pnt[2],p00,p01,p02,p1,p2,0,1)-psi_a_dx(pnt[0],pnt[1],pnt[2]))**2 +
                           (dN_dxj_k(pnt[0],pnt[1],pnt[2],p00,p01,p02,p1,p2,1,1)-psi_a_dy(pnt[0],pnt[1],pnt[2]))**2 +
                           (dN_dxj_k(pnt[0],pnt[1],pnt[2],p00,p01,p02,p1,p2,2,1)-psi_a_dz(pnt[0],pnt[1],pnt[2]))**2)

    for i, pnt in enumerate(all_points):
        ana1[i] = dOmega * (np.abs(psi_a(pnt[0], pnt[1], pnt[2]))**2)
        
    for i, pnt in enumerate(all_points):
        ana2[i] = dOmega* (psi_a_dx(pnt[0], pnt[1], pnt[2])**2 + psi_a_dy(pnt[0], pnt[1], pnt[2])**2 + 
                           psi_a_dz(pnt[0], pnt[1], pnt[2])**2)
    
    rel_err = (np.sum(tr1) + np.sum(tr2))**(1/2) / (np.sum(ana1) + np.sum(ana2))**(1/2)

    return rel_err

def relative_err_without_points(p00, p01, p02, p1, p2, dx_r, dx_th1, dx_th2):
    ...
    all_points = np.array(list(zip(x,y,zz)))
    return relative_err(p00, p01, p02, p1, p2, all_points)
...
# %%
@njit
def dE_dwj(points, boundary_points, p00, p01, p02, p1, p2, j, BC):
    t1 = np.zeros((points.shape[0], p00.shape[0]))
    t2 = np.zeros((boundary_points.shape[0], p00.shape[0]))
    
    for i, pnt in enumerate(points):
        t1[i] = 2 * error_term1(pnt[0],pnt[1],pnt[2],p00, p01, p02, p1, p2) * (
                d_dwj_dN2_dxk2(pnt[0],pnt[1],pnt[2],p00, p01, p02, p1, p2,j,0) + 
                d_dwj_dN2_dxk2(pnt[0],pnt[1],pnt[2],p00, p01, p02, p1, p2,j,1) +
                d_dwj_dN2_dxk2(pnt[0],pnt[1],pnt[2],p00, p01, p02, p1, p2,j,2))
        
    for i, pnt in enumerate(boundary_points):
        t2[i] = 2 * N(pnt[0],pnt[1],pnt[2],p00, p01, p02, p1, p2) * \
                dN_dwj(pnt[0],pnt[1],pnt[2],p00, p01, p02, p1, p2,j)
...
@njit
def dE_du(points, boundary_points, p00, p01, p02, p1, p2, BC):
    t1 = np.zeros((points.shape[0], p1.shape[0]))
    t2 = np.zeros((boundary_points.shape[0], p1.shape[0]))
    
    for i, pnt in enumerate(points):
        t1[i] = 2 * error_term1(pnt[0],pnt[1],pnt[2],p00, p01, p02, p1, p2) * (
                d_du_dkN(pnt[0],pnt[1],pnt[2],p00, p01, p02, p1, p2,0,2) + 
                d_du_dkN(pnt[0],pnt[1],pnt[2],p00, p01, p02, p1, p2,1,2) +
                d_du_dkN(pnt[0],pnt[1],pnt[2],p00, p01, p02, p1, p2,2,2))
        
    for i, pnt in enumerate(boundary_points):
        t2[i] = 2 * N(pnt[0],pnt[1],pnt[2],p00, p01, p02, p1, p2) * \
                d_du_dkN(pnt[0],pnt[1],pnt[2],p00, p01, p02, p1, p2,0,0)
...
@njit
def dE_dv(points, boundary_points, p00, p01, p02, p1, p2, BC):
    t1 = np.zeros((points.shape[0], p2.shape[0]))
    t2 = np.zeros((boundary_points.shape[0], p2.shape[0]))
    
    for i, pnt in enumerate(points):
        t1[i] = 2 * error_term1(pnt[0],pnt[1],pnt[2],p00, p01, p02, p1, p2) * (
                d_dv_dkN(pnt[0],pnt[1],pnt[2],p00, p01, p02, p1, p2,0,2) +
                d_dv_dkN(pnt[0],pnt[1],pnt[2],p00, p01, p02, p1, p2,1,2) +
                d_dv_dkN(pnt[0],pnt[1],pnt[2],p00, p01, p02, p1, p2,2,2))
        
    for i, pnt in enumerate(boundary_points):
        t2[i] = 2 * N(pnt[0],pnt[1],pnt[2],p00, p01, p02, p1, p2) *\
                d_dv_dkN(pnt[0],pnt[1],pnt[2],p00, p01, p02, p1, p2,0,0)
